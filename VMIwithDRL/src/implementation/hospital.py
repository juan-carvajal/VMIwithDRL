...
#                 inventory_aux[i]=inventory_aux[i+1]
        ...
        for i, val in enumerate(self.inventory): 
            if i == 0:
                 ...
                 inventory_aux[i] = max(0, self.inventory[i + 1] + supply[i] - max(0, demand - (sum(self.inventory[:i + 1]) + sum(int(v) for v in supply[:i + 1]))))
            else: inventory_aux[i] = max(0,supply[i] - max(0, demand - ( sum(self.inventory[:i + 1]) + sum(int(v) for v in supply[:i + 1]))))
