...
from agent_model.ClippedDDQN_agent_episode import TrainingAgent
...
if __name__ == '__main__':
    hosp_inv = [0] * 20
    initial_state = [10, 10, 10, 10, 10, 1] + hosp_inv+[0]
    ...
    agent = TrainingAgent(model=model, runs=train_runs, steps_per_run=365, batch_size=10, memory=100000, use_gpu=True,
                          epsilon_function='constant', min_epsilon=0.2, epsilon_min_percentage=0.05,train_frame="step",train_period=1,lr=5e-6)
    rewards = agent.run()
    validate_runs = 500
