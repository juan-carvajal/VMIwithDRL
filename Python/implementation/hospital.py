class Hospital():
    ...
    def supply(self, supply, demand):
        ...
        stockout = max(0, demand - (sum(self.inventory) + sum(supply)))
        for i, val in enumerate(self.inventory):
            if i == 0:
                self.inventory_aux[i] = max(0, self.inventory[i + 1] + supply[i + 1] - demand)
            elif 0 < i < 4:
                self.inventory_aux[i] = max(0, self.inventory[i + 1] + supply[i]
                                            - max(0, demand - (sum(self.inventory[:i + 1]) + sum(supply[:i + 1]))))
            else:
                inventory_aux[i] = max(0,
                                       supply[i] - max(0, demand - (sum(self.inventory[:i + 1]) + sum(supply[:i + 1]))))
